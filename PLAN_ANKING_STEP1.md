...
> **Estado (2026-07-09, actualizado):** v1 en marcha. App migrada, traducida al inglés, tema visual rojo, icono propio, desplegada en Vercel. Datos en producción (`app/data/db.js`): **114 temas / 11.088 preguntas / 2 fuentes** (AnKing + cuadernillo oficial NBME/FSMB), de las cuales **81 llevan imagen** (21 del cuadernillo NBME + 60 de una tanda piloto de AnKing "con imagen" — ver §13, incluida una segunda pasada de reparación que recuperó 13 de las 14 descartadas inicialmente). Explicaciones de la cohorte antigua de AnKing (fase 1 + 1ª tanda hiperloop, prompt sin reforzar) ya corregidas casi al completo — ver §10.7. Esquema ampliado para soportar opciones de longitud variable (hasta G) e imágenes en el enunciado — ver §11. Nuevas features de práctica: "Practice all subjects" (sin filtro de tema) y filtro "solo preguntas con imagen" en Advanced Options — ver §12. Pantalla "Subjects" ya no fragmenta temas homónimos entre fuentes (AnKing/NBME fusionados en un solo nodo con selector de fuente). **Decimotercera tanda de conversión de texto** (250/250, ver §21): 11.088 → 11.338, primera vez cortando ya excluyendo `_blacklist_anki.json` — 0 flagged por QA. Quedan **~7.028** tarjetas limpias de AnKing (universo de solo texto) por convertir, más el universo de imagen/audio tratado en §13.
...
## 19. Undécima tanda de conversión AnKing de texto (500 preguntas, 2026-07-09)

> Nota de numeración: el §15 ("Séptima tanda") quedó desplazado más abajo en el documento porque una sesión posterior insertó los §16-18 justo después del §14 en vez de después del §15. Se deja tal cual (contenido correcto, solo el orden de aparición es raro) para no arriesgar un merge manual del texto; los números de sección siguen siendo únicos y sirven de referencia.

Esta tanda se hizo en la máquina de "arnal" (las tandas 8ª-10ª, §16-18, se habían hecho en la máquina "ca-urgencias" — carpetas `_batches_in_v7/v8/v9` no existen en este disco, son locales/gitignored de la otra máquina). El usuario pidió `git pull` (ya estaba al día, 9.601 preguntas) y 500 preguntas más, sin querer confirmaciones adicionales a mitad de camino.

- `BASE` del workflow actualizado a la ruta de esta máquina (`C:/Users/arnal/Desktop/Documentos/IA/USMLE-Step-1/data/anking`). `TAXONOMY` regenerada desde cero a partir de los 114 temas reales en producción (72 nombres únicos tras deduplicar por fuente) — incluye los 4 temas nuevos de las tandas 9ª/10ª (`Gastrointestinal Embryology`, `Mycology`, `Basic Microbiology`, `Bacterial Growth & Genetics`) que no estaban en la lista hardcodeada.
- Recalculado sobre `_raw_all.json` (18.189) menos los `anki` ya usados en `step1_dataset_9601q.json` (9.481) → **8.771 restantes**; cortados 20 lotes de 25 en `_batches_in_v11` (carpeta nueva, sin colisión con las v7-v9 de la otra máquina).
- Mismo bug de CRLF de siempre al reeditar el script en Windows — normalizado a LF antes de lanzar.
- **20/20 lotes verificados en disco** (500 preguntas, 0 errores de agente). QA marcó 5: 2 por contenido factualmente invertido (achalasia SCC-vs-adenocarcinoma; Sister Mary Joseph nodule intestinal-vs-diffuso), 1 por holoprosencefalia con tálamos-vs-ganglios basales invertidos, y 2 por `tema_nombre`/`sistema` no coincidiendo con el contenido real de la pregunta (Guillain-Barré etiquetado como Anatomy & Physiology en vez de Pathology; embriología de la duramadre etiquetada como Reproductive System Embryology). Fichero: `data/anking/_flagged_v11.json`.
- Fusión (`--base step1_dataset_9601q.json --glob "_batches_out_v11/*.json" --flagged _flagged_v11.json`): **495 aceptadas / 5 rechazadas** (todas por QA, 0 rechazos estructurales). Total: **9.601 → 10.096 preguntas, 114 temas** (sin temas nuevos — la taxonomía regenerada evitó duplicados).
- **Auditoría de `temas[]` (rutina desde §15.2/§16.2/§18): limpia** — 0 nombres corruptos con corchete/número, 0 duplicados reales por `fuente`+`nombre`, 0 huérfanos, revisión manual de nombres por sistema sin near-duplicados semánticos nuevos (tipo "Bacterial Growth & Genetics" vs "Bacterial Genetics" de §18).
- `DB_HASH` → `step1-10096q`. Verificado en preview (cachés/SW limpiados): `window.DB.preguntas.length === 10096`, `temas === 114`, pantalla Subjects sin duplicados (72 nodos visibles agrupando 114 ids), práctica de una pregunta real (Antimicrobials, cloranfenicol/"Gray Baby") renderiza y corrige bien.
- **Quedan ~8.276 tarjetas limpias de AnKing (texto) por convertir** (8.771 − 495). Para la próxima tanda: cortar `_batches_in_v12` recalculando sobre `step1_dataset_10096q.json`.

---

## 20. Duodécima tanda de conversión AnKing de texto (1.000 preguntas, 2026-07-09)

Misma receta de §19 (máquina "arnal", `BASE`/`TAXONOMY` ya correctos de la tanda anterior, sin necesitar ajustes). El usuario pidió explícitamente no ser interrumpido con más confirmaciones a mitad de tanda — se ejecutó de principio a fin (generación, fusión, auditoría, commit, push) sin pausas.

- Recalculado sobre `_raw_all.json` menos los `anki` ya usados en `step1_dataset_10096q.json` (9.976) → **8.276 restantes**; cortados 40 lotes de 25 en `_batches_in_v12`.
- **40/40 lotes verificados en disco** (999 preguntas, 0 errores de agente — un agente soltó 1 pregunta). QA marcó 6, de las cuales **3 son exactamente los mismos `anki` ids ya marcados en la tanda anterior** (§19: achalasia SCC/adenocarcinoma, Sister Mary Joseph nodule intestinal-vs-diffuso, holoprosencefalia tálamos-vs-ganglios basales) — **hallazgo nuevo importante: una tarjeta descartada por QA sigue en el "universo limpio" y puede volver a salir sorteada en una tanda futura**, reproduciendo el mismo error de contenido porque es la misma tarjeta fuente ambigua/mal etiquetada, no un fallo aleatorio del modelo. Las otras 3 nuevas: vilazodona con perfil de efectos adversos mal descrito, causa más frecuente de hepatopatía crónica (alcohol vs. NAFLD/MASLD desactualizado), y una pregunta de bifurcación de aorta abdominal mal etiquetada como `sistema: "Respiratory"`.
- **Mitigación aplicada**: se creó `data/anking/_blacklist_anki.json`, agregando los `anki` ids de todos los `_flagged_*.json` disponibles en disco (waves 1-3 + v11 + v12 — los de v7/v8/v9, hechos en la otra máquina, no se pudieron recuperar por estar gitignored). **22 ids únicos.** Este fichero SÍ se versiona en git (a diferencia de `_flagged_v*.json`, que son gitignored por tanda). **Para la próxima tanda: al recalcular "lo que falta", excluir también los `anki` de este fichero del pool de `_raw_all.json`** (no solo los `done_ids` del dataset), para no volver a generar estas tarjetas problemáticas ni gastar agentes en ellas.
- Fusión (`--base step1_dataset_10096q.json --glob "_batches_out_v12/*.json" --flagged _flagged_v12.json`): **992 aceptadas / 7 rechazadas** (6 QA + 1 estructural por letra desalineada). Total: **10.096 → 11.088 preguntas, 114 temas** (sin temas nuevos).
- Auditoría de `temas[]`: limpia (0 nombres corruptos, 0 duplicados reales, 0 huérfanos, sin near-duplicados semánticos nuevos).
- `DB_HASH` → `step1-11088q`. Verificado en preview (cachés/SW limpiados): `window.DB.preguntas.length === 11088`, `temas === 114`, pantalla Subjects sin duplicados (72 nodos), sin errores de consola.
- **Quedan ~7.284 tarjetas limpias de AnKing (texto) por convertir** (8.276 − 992). Para la próxima tanda: cortar `_batches_in_v13` recalculando sobre `step1_dataset_11088q.json` **y excluyendo también los ids de `_blacklist_anki.json`**.

---

## 21. Decimotercera tanda de conversión AnKing de texto (250 preguntas, 2026-07-09)

Misma receta de §20, primera vez aplicando la mitigación descrita ahí: al recalcular "lo que falta" se excluyeron también los 22 `anki` ids de `data/anking/_blacklist_anki.json`, no solo los ya usados en producción.

- Recalculado sobre `_raw_all.json` menos `done_ids` (10.968, de `step1_dataset_11088q.json`) menos blacklist (22) → **7.278 restantes**; cortados 10 lotes de 25 en `_batches_in_v13`.
- **10/10 lotes verificados en disco, 250/250 preguntas, 0 errores de agente, 0 flagged por QA** — la tanda más limpia hasta ahora (sin incidencias de ningún tipo).
- Fusión (`--base step1_dataset_11088q.json --glob "_batches_out_v13/*.json"`, sin `--flagged`): **250 aceptadas / 0 rechazadas**. Total: **11.088 → 11.338 preguntas, 114 temas** (sin temas nuevos).
- Auditoría de `temas[]`: limpia (0 nombres corruptos, 0 duplicados reales, 0 huérfanos).
- `DB_HASH` → `step1-11338q`. Verificado en preview (cachés/SW limpiados): `window.DB.preguntas.length === 11338`, `temas === 114`, pantalla Subjects sin duplicados (72 nodos), sin errores de consola.
- **Quedan ~7.028 tarjetas limpias de AnKing (texto) por convertir** (7.278 − 250). Para la próxima tanda: cortar `_batches_in_v14` recalculando sobre `step1_dataset_11338q.json`, excluyendo `_blacklist_anki.json` (y añadiendo a ese fichero cualquier `anki` nuevo que marque QA en tandas futuras).
